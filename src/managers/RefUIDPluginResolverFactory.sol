...
contract RefUidPluginResolverFactory {
    ...
    event ResolverDeployed(address indexed deployer, RefUidPluginResolver indexed deployedContract);

    function deploy(bytes32 _salt, address _eas) external returns (RefUidPluginResolver) {
        RefUidPluginResolver resolver = new RefUidPluginResolver{salt: _salt}(_eas);
