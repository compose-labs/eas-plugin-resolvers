...
library EnumerableExecutingResolverSet {
    ...
    function at(
        Set storage set,
        uint256 index
    ) internal view returns (IExecutingResolver) {
        return
            IExecutingResolver(address(uint160(uint256(set._inner.at(index)))));
    }
    ...
    function values(
        Set storage set
    ) internal view returns (IExecutingResolver[] memory) {
        bytes32[] memory store = set._inner.values();
        IExecutingResolver[] memory result = new IExecutingResolver[](
            store.length
        );
